...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter{
	 ...
	 *
	 * http.antMatchers("/resources/**","/error","/api/user/**").permitAll()
	 * antMatchers("/api/student/**").hasRole("STUDENT");
	 * antMatchers("/api/teacher/**").hasRole("TEACHER");
	 * antMatchers("/api/manager/**").hasRole("MANAGER");
	 * request.anyRequest().fullyAuthenticated().and();
	 *
	 * http.logout().permitAll(); http.logoutRequestMatchers(new
	 * AntPathRequestMatcher("/api/user/logout","POST")); http.and();
	 *
	 *
	 * http.formLogin().loginPage("/api/user/login").and(); http.httpBasic().and();
	 *
	 * http.csrf().disable(); }
	 */
